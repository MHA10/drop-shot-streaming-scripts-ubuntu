...
if (!SUPABASE_URL || !SUPABASE_KEY) {
  console.error('[FATAL] Set SUPABASE_URL and SUPABASE_SERVICE_KEY (service_role) in the environment.');
  process.exit(1);
}
...
function onLine(portPath, rawLine) {
  ...
  void updateSupabase(p.courtId, p.scoreA, p.scoreB, p.gamesA, p.gamesB);
}
...
async function scan() {
  ...
  const esp = ports.filter(p =>
    (!PORT_FILTER || p.path.includes(PORT_FILTER) || (p.serialNumber && p.serialNumber.includes(PORT_FILTER))) &&
    ((p.manufacturer && (/Silicon Labs|wch\.cn|QinHeng/.test(p.manufacturer))) ||
      p.vendorId === '1a86' || /usbserial|ttyUSB|ttyACM/.test(p.path)));
